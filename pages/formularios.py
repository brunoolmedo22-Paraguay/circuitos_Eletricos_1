...
def toggle_formula(formula_id: str) -> None:
    """Abre/cierra el detalle dentro de la propia tarjeta de la fórmula."""
...
def set_mode(mode_name: str) -> None:
    ...
    # El detalle pertenece al contexto de la grilla visible. Al cambiar de
    # consulta evitamos mantener una tarjeta "abierta" fuera de pantalla.
    st.session_state.formula_selected_id = None


def render_inline_details(formula: dict) -> None:
    """Renderiza los detalles dentro del panel de la fórmula seleccionada."""
    st.markdown("<div class='formula-card-detail-kicker'>DETALLES</div>", unsafe_allow_html=True)
    ...
    st.caption(
        ...
        f"Sección {formula['section']} · p. {formula['pages']}"
    )
        ...
        for idx, formula in enumerate(rows):
            ...
            with grid[idx % 3]:
                ...
                with st.container(key=card_key):
                    ...
                    st.button(
                        "Ocultar detalles" if is_open else "Ver detalles",
